...
module Privat24
  ...
  private
    ...
    #     raise Private24::Exception.new(required_field + ' is a required field') unless self.send(required_field).to_s != ''
    #   end
    #
    #   raise Private24::Exception.new('currency must be one of '+Private24::SUPPORTED_CURRENCIES.join(', ')) unless Private24::SUPPORTED_CURRENCIES.include?(currency)
    #
    #   begin
    #     self.amount = Float(self.amount)
    #   rescue ArgumentError, TypeError
    #     raise Private24::Exception.new('amount must be a number')
    #   end
    #
    #   raise Private24::Exception.new('amount must be rounded to 2 decimal digits') unless self.amount.round(2) == self.amount
    #
    #   raise Private24::Exception.new('amount must be more than 0.01') unless amount > 0.01
    # end
  end
end
